...
func replaceInlineTypes(src GoSchema, options ParseOptions) GoSchema {
	if len(src.Properties) == 0 || src.RefType != "" {
		return src
	}

	currentTypes := options.currentTypes
	name := schemaNameToTypeName(pathToTypeName(options.path))
	if _, exists := currentTypes[name]; exists {
		name = generateTypeName(currentTypes, name, []string{})
	}
